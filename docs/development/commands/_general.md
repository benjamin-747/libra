...
- Git 兼容命令以 `COMPATIBILITY.md` 为用户承诺，以 `docs/development/commands/<cmd>.md` 记录实现细节和未完成项，并以 `docs/development/integration/integration-test-plan.md` / `docs/development/integration-scenarios.yaml` 作为集成验证方案的事实源。
...
- 执行路径：维护者新增或修改命令文档时，先核对 `src/cli.rs`、`src/command/<cmd>.rs` 和必要的子模块，再把入口、参数类型、输出/错误类型、执行流程和测试证据写回对应文档；若修改的是 Git 兼容命令，还必须同步 `COMPATIBILITY.md`、`docs/development/integration/integration-test-plan.md`、`docs/development/integration-scenarios.yaml` 和对应集成测试证据，保证命令文档、用户可见承诺和集成测试方案一致。
...
- 修改 Git 兼容命令时，必须把新增、删除或语义变化的场景加入 `docs/development/integration/integration-test-plan.md` 和 `docs/development/integration-scenarios.yaml`，并保持对应 runner、Wave、执行命令和验收证据一致。
