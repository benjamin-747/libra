...
> **2026-07-08 A0-01 现状复核**：对下表逐项做了源码核对（10 路并行调查，回指 `file:line` 锚点），确认本表**据实、无超前声明**。复核结论的任务状态表（哪些是 `继续实现`、哪些是 `文档守卫`）记录在 [`docs/development/plan/plan-20260708.md`](../plan/plan-20260708.md) 的「A0-01 现状复核结果」小节：真实实现项为 A0-02/03/04/06/07/08/09，文档守卫项为 A0-05/10/11。任一表项状态变化时，两处必须同步刷新。
...
| Hook 生命周期 | `CheckpointScope::Subagent` 独立 checkpoint 物化 | `LifecycleEventKind` 已补 `SubagentStart`/`SubagentEnd`，Codex provider 已映射原生 subagent hooks，事件写入 session metadata；尚未把 subagent boundary 物化为独立 `scope='subagent'` checkpoint。 | entire `lifecycle.go`,`agent/codex/trust.go` / future checkpoint scope |
...
| Skill | full agent-specific skill discovery registry | AG-21 `SkillEvent` **extraction 已落地**（进入 checkpoint 提取 metadata，`runtime.rs`）；**尚无 searchable session/checkpoint/search 投影**，独立 `SkillDiscoverer` registry 亦未公开。 | entire `agent/skilldiscovery/`,`skill_events*` / future parity |
