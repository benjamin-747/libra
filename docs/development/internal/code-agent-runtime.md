...
`libra agent` 需要的 public CLI/API、E1-E9 wire 契约、AG-16~AG-24 任务卡、checkpoint/export、review/investigate 和验收命令已经拆入 [`docs/development/tracing/agent.md`](commands/agent.md)。本节只保留 Gate 8 与内部 AgentRuntime/Web-only 迁移的边界关系、来源证据摘要和历史背景，避免 `libra agent` 的外部捕获契约继续散落在两个文档。

> **AG-16~AG-21 落地实施计划（统一 Provider 接口 trait 草图 + 逐文件改动清单）** 见 [`docs/development/tracing/agent.md`](commands/agent.md) 的「AG-16~AG-21 统一 Provider 接口落地实施计划」节。要点：删除 dead 的 `ObservedAgentHooks`、hook 能力收敛到 `ObservedAgent::as_hooks() -> Option<&dyn HookProvider>` 并以 `AgentKind` 取代字符串 `find_provider`、落地 `DeclaredAgentCaps`/`CapabilityDeclarer` 8-bool（E1）、补齐 E1 可选能力 trait、`LifecycleEventKind` 补 `SubagentStart`/`SubagentEnd`。**该计划只动 provider/capability/lifecycle 轴，与存储对象模型解耦**：外部捕获保持 raw SQL `agent_*` + `refs/libra/traces` 的 `blob/tree/commit`，**不**迁到 git-internal AI 对象模型（`is_ai_object()` 族；该模型仅内部 AgentRuntime 使用）。该节与本节为同一计划的两侧镜像，任一处改动须同 PR 同步。
...
- **内部 AgentRuntime 仍是本文主线。** `libra code` 的 turn submit/respond/cancel/observe 只走 WebSocket/Web API；MCP stdio 仍由 [`mcp.md`](mcp.md) 独立承接；`libra code-control` 只是 Web-only 迁移前后的 automation shim 讨论对象。
...
> 本块保留为 Gate 8 的内部计划镜像；`libra agent` public contract 的权威副本在 [`docs/development/tracing/agent.md`](commands/agent.md) 的「已验证 wire 契约（E1-E9）」节。凡卡正文/Phase 出现「命名可调整」「至少包含」「等价 contract」，都以 commands/agent.md 的精确标识符为准；本节变化必须同步到 commands/agent.md，反之亦然。
>
> **验证审计（2026-06-16）**：本节统计数字来自对 `/Volumes/Data/entireio/cli-checkpoints` 的实地遍历——根 `metadata.json` 4,377 个、per-session `metadata.json` 5,949 个、最大单 checkpoint 61 session、最大 `full.jsonl` 48,774,003 bytes（46.51 MiB）、唯一 `.zst` 路径 `e6/5d9bec561a/0/full.jsonl.zst`。计数方法见下方「快速刷新协议」第 2 步；未来复跑若数字变化，必须在本段打补丁并同步 `commands/agent.md`。
>
...
# 4. 两份文档一致性
rg -n "AG-1[6-9]|AG-2[0-4]|claudecode|SubagentStart|content_hash|DeclaredCaps|libra-agent-.*info" docs/development/code-agent-runtime.md docs/development/tracing/agent.md | sort | uniq -c | awk '$1!=2{print "DRIFT:",$0}'
```
...
[`docs/development/tracing/agent.md`](commands/agent.md) 是 `libra agent` 外部捕获的 public CLI/API source-of-truth：它承接从本文拆出的 E1-E9 wire 契约、AG-16~AG-24 任务卡、checkpoint/export、review/investigate、验收命令和“还未实现”表。

- 本文继续负责内部 `libra code` AgentRuntime / Web-only 迁移主线，以及 Gate 8 是否与内部 runtime、MCP、control-plane 边界保持正交。
- Gate 8 仍可在本文的总体门禁、任务拆分矩阵和完成定义中出现，但实现者领取 AG-16~AG-24 时，必须先读 `commands/agent.md` 的 public contract。
- 两份文档同步规则：`commands/agent.md` 改 public surface、E 契约、AG-16~AG-24 或有意差异时，同步刷新本文 Gate 8 摘要；本文若改变 Gate 8 边界或内部 AgentRuntime 约束，也同步刷新 `commands/agent.md` 的边界说明。
...
- **任务卡不可偷换 scope。** AG-00 只改本文；AG-06 只检查 `mcp.md` 给出的独立 MCP 命令边界；AG-13~AG-15 不允许顺手切默认 Web 或删除 TUI；AG-16~AG-24 不允许把 external capture 与内部 AgentRuntime/MCP 控制面合并。
...
目标：把 Gate 8 的 public behavior、JSON schema、fixtures 和用户文档收敛，避免 `docs/development/code-agent-runtime.md`、`docs/development/tracing/agent.md`、`docs/commands/agent.md` 三处继续漂移。

依赖：AG-16~AG-23。

任务：

- 更新 [`docs/development/tracing/agent.md`](commands/agent.md)：成为 `libra agent` 外部捕获的权威 CLI/API 文档，包含 capability matrix、list/add/remove alias、RPC protocol、lifecycle event、checkpoint/export shape、review/investigate 用户流。
...
- `docs/development/code-agent-runtime.md` 与 `docs/development/tracing/agent.md` 的边界说明一致：前者是内部 AgentRuntime + Web-only 迁移 + Gate 8 总览追踪，后者是 `libra agent` public CLI/API/source-of-truth（含 E1-E9、AG-16~AG-24、checkpoint/export、review/investigate、验收命令和“还未实现”表）。**任何一处修改 AG 卡、E 契约、已知差距或有意差异，必须同一 PR 内双向同步**（见 AG-24 强制同步规则）。
...
```
检查本文档内部链接是否有效（`mcp.md`、`docs/development/tracing/agent.md` 等）。
...
* Public docs: `docs/commands/agent.md`、[`docs/development/tracing/agent.md`](commands/agent.md)
...
* Protocol docs: [`docs/development/tracing/agent.md`](commands/agent.md)
...
- 验收时跑 `rg -n "AG-1[6-9]|AG-2[0-4]|DeclaredAgentCaps|libra-agent-.*info|SubagentStart|CheckpointSummary|content_hash" docs/development/code-agent-runtime.md docs/development/tracing/agent.md` 确保一致；任何漂移视为 AG-24 未完成。

##### 1. 涉及文件
* 本文件: [`docs/development/code-agent-runtime.md`](code-agent-runtime.md)
* Public agent plan: [`docs/development/tracing/agent.md`](commands/agent.md)
...
| 外部 agent 捕获 | 无（但有 Gemini-CLI MCP 扩展把 GAIT 当后端） | `libra agent` 捕获 Claude/Codex hook + transcript（见 [`commands/agent.md`](commands/agent.md)） |
| MCP | 核心无；扩展经 MCP 以 GAIT 为后端 | `libra mcp` server（见 [`mcp.md`](mcp.md)）+ 统一 provider 接口计划 |
...
> 边界提醒：建议 1–4 属内部 `libra code` 会话面 / Memory 改进,与外部捕获面(`commands/agent.md`)、git-internal AI 对象模型解耦;建议 5 横跨 `libra mcp` 与 observed-agents,须先确认 MCP 边界(`mcp.md` 的 Boundaries:mutating 工具经 `McpAuthorizer`、不作为 agent 控制面)。
