...
1. 对输入执行 owner filtering、路径规范化、大小限制、redaction 与 worthiness 检查。
...
这些都是投影，**不是**历史真相。若被丢弃，可从 `refs/libra/memory/...` 重建。
...
在访问模式上还有一条对齐约定值得明确：Memory 的投影表用 SeaORM entity 来访问（与同样可重建的 `ai_index_*` 投影一致），而不采用 `agent_session` / `agent_checkpoint` / `agent_usage_stats` 那种**故意**保持的 raw-SQL、无 entity 风格。原因在于：`agent_*` 那批表是外部捕获的独立账本，而 Memory 的这些表是 git 真源（`refs/libra/memory/...`）的可重建投影，本质与 `ai_index_*` 同类，因而对齐 `ai_index_*` 的 SeaORM 模式。
...
| `memory_inspect_injection` | 展示最近一次或当前 prompt 注入内容 |
...
- **可重放。** 删除所有 `memory_*` 投影表后，`libra memory rebuild` 必须从 Git 历史确定性恢复同一 live head 集合；若遇到损坏 blob，应跳过该 commit 的受影响对象、记录诊断并继续重建其它对象。
...
所有投影表都允许被整体删除后，再由 `libra memory rebuild` 从
...
退出门槛：路径聚合可用；投影重建逐字节稳定；分支切换不会让 memory
跨 ref 泄漏。
...
- prompt 期注入（§11.7），使用 `ProjectMemory` 与
  `MemoryAnchor` 预算分段。
- 用于可观测性的 `libra memory inspect-injection`。
